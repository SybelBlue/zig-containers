...
pub fn Chunk(comptime A: type, comptime n: usize) type {
    return struct {
        ...
        pub fn popBack(self: *Self) error{Empty}!A {
            if (self.isEmpty()) return error.Empty;
            defer self.right -= 1;
            return self.forceRead(self.right);
        }

        pub fn popFront(self: *Self) error{Empty}!A {
            if (self.isEmpty()) return error.Empty;
            defer self.left += 1;
        ...
        pub fn splitOff(self: *Self, index: usize) Self {
            ...
            var right_chunk = .{};
            if (index == self.len()) return right_chunk;
        ...
        pub fn remove(self: *Self, index: usize) A {
            if (index >= self.len()) @panic("Chunk.remove: index out of bounds");
            const real_index = index + self.left;
...
test "issue_11_testcase3a: clone with panic drops correctly (miri)" {
    ...
    chunk.pushBack(DropDetector.init(42));
    chunk.pushBack(DropDetector.init(42));
    chunk.pushBack(DropDetector.init(43));
    _ = chunk.popFront();
...
test "issue_11_testcase3b: insert_from with panicking iterator drops correctly" {
    const result = std.testing.expectPanic(struct {
        fn call() void {
            ...
            chunk.pushBack(DropDetector.init(1));
            chunk.pushBack(DropDetector.init(2));
            chunk.pushBack(DropDetector.init(3));
            var it = PanickingIterator{ .current = 1, .panic_at = 1, .len = 1 };
            chunk.insertFrom(1, &it);
        }
    }.call);
    _ = result;
}
...
test "is_full" {
    var chunk = Chunk(i32, 64).init();
    for (0..64) |i| {
        try testing.expectEqual(false, chunk.isFull());
        chunk.pushBack(@intCast(i));
    }
...
test "push_back_front" {
    var chunk = Chunk(i32, 64).init();
    for (12..20) |i| chunk.pushBack(@intCast(i));
    try testing.expectEqual(@as(usize, 8), chunk.len());

    var i: i32 = 11;
    while (i >= 0) : (i -= 1) chunk.pushFront(i);
    try testing.expectEqual(@as(usize, 20), chunk.len());

    for (20..32) |j| chunk.pushBack(@intCast(j));
    try testing.expectEqual(@as(usize, 32), chunk.len());

    var out = std.ArrayList(i32).init(testing.allocator);
    defer out.deinit();
    var it = chunk.iter();
    while (it.next()) |v| try out.append(v.*);
...
test "split_off" {
    var left = Chunk(i32, 64).init();
    for (0..6) |i| left.pushBack(@intCast(i));
    var right = left.splitOff(3);

    var left_out = std.ArrayList(i32).init(testing.allocator);
    defer left_out.deinit();
    var right_out = std.ArrayList(i32).init(testing.allocator);
    defer right_out.deinit();

    var it = left.iter();
    while (it.next()) |v| try left_out.append(v.*);
    it = right.iter();
    while (it.next()) |v| try right_out.append(v.*);
...
test "append" {
    var left = Chunk(i32, 64).init();
    for (0..32) |i| left.pushBack(@intCast(i));

    var right = Chunk(i32, 64).init();
    var i: i32 = 63;
    while (i >= 32) : (i -= 1) right.pushFront(i);
...
test "insert_middle" {
    var chunk = Chunk(i32, 64).init();
    for (0..32) |i| chunk.pushBack(@intCast(i));
    for (33..64) |i| chunk.pushBack(@intCast(i));
    chunk.insert(32, 32);
...
test "insert_back" {
    var chunk = Chunk(i32, 64).init();
    for (0..63) |i| chunk.pushBack(@intCast(i));
    chunk.insert(63, 63);
...
test "insert_front" {
    var chunk = Chunk(i32, 64).init();
    var i: usize = 1;
    while (i < 64) : (i += 1) chunk.pushFront(@intCast(64 - i));
    chunk.insert(0, 0);
...
test "remove_value" {
    ...
    var expected = std.ArrayList(i32).init(testing.allocator);
    defer expected.deinit();
    for (0..32) |i| try expected.append(@intCast(i));
    for (33..64) |i| try expected.append(@intCast(i));
...
test "dropping: all elements dropped correctly" {
...

    {
        ...
        for (0..20) |_| chunk.pushBack(DropTest.init(&counter));
        for (0..20) |_| chunk.pushFront(DropTest.init(&counter));
        try testing.expectEqual(@as(usize, 40), counter.load(.relaxed));
...
test "unit_on_empty: unit with CAPACITY 0 panics" {
    try testing.expectPanic(struct {
        fn call() void {
            _ = Chunk(usize, 0).unit(1);
        }
    }.call);
}

test "pair_on_empty: pair with CAPACITY 0 panics" {
    try testing.expectPanic(struct {
        fn call() void {
            _ = Chunk(usize, 0).pair(1, 2);
        }
    }.call);
}
